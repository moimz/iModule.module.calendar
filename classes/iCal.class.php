...
class iCal {
    ...
    /**
     * The value in month to use for indefinite, recurring events
     *
     * @var integer
     */
     ...
     * @return void
     */
    protected function initLines($lines)
    {
        ...
        if (!empty($events)) {
            foreach ($events as $anEvent) {
                if (isset($anEvent['RRULE']) && $anEvent['RRULE'] !== '') {
                    ...
                    $is_allday = strlen($anEvent['DTSTART_array'][1]) == 8 && strlen($anEvent['DTEND_array'][1]) == 8;
                    ...
                    if (is_int($this->defaultSpan)) {
                        $untilDefault = date_create($this->startDate == null ? 'now' : $this->startDate);
                        $untilDefault->modify($this->defaultSpan . ' months');
                    ...
                    if (isset($rrules['UNTIL'])) {
                        ...
                        if ($until > strtotime('+' . $this->defaultSpan  . ' months')) {
                            $until = strtotime('+' . $this->defaultSpan  . ' months');
                        }
                    ...
                    switch ($frequency) {
                        case 'DAILY':
                            ...
                            while ($recurringTimestamp <= $until) {
                                ...
                                if ($is_allday == true) $anEvent['DTSTART'] = substr($anEvent['DTSTART'],0,8);
                                ...
                                $anEvent['DTEND'] = date(self::DATE_TIME_FORMAT, $anEvent['DTEND_array'][2]) . (($initialEndTimeZoneName === 'Z') ? 'Z' : '');
                                if ($is_allday == true) $anEvent['DTEND'] = substr($anEvent['DTEND'],0,8);
